// @ts-nocheck
import { OutputException, StopException } from './exceptions.js';
export class LogoInterpreter {
    constructor() {
        ...
        this.turtle3D = null; // 3D turtle representation
        this.faceVertices = []; // For BEGINFACE/ENDFACE
        this.faceColor = '#00ff00';
        this.faceColorRGB = [0, 255, 0];
        this.meshHistory = []; // Track created meshes for CSG operations
        this.lastMesh = null; // Reference to most recently created mesh
    ...
    setup3DView() {
        ...
        if (exportSTLButton)
            exportSTLButton.style.display = 'inline-block';
    ...
    reset() {
        ...
        this.faceVertices = []; // Clear face vertices
    ...
    clear() {
        ...
        // Clear 3D scene if in 3D mode
        if (this.is3DMode && this.scene3D) {
            // Remove all mesh and line objects from scene, keep helpers and lights
            const objectsToRemove = [];
            this.scene3D.children.forEach(child => {
                if (child.type === 'Line' || child.type === 'Mesh' && child !== this.turtle3D) {
    ...
    applyZoomAndPan() {
        if (!this.canvas)
            return; // Skip if DOM elements not available
    ...
    updateTurtleDisplay() {
        ...
        this.turtleElement.setAttribute('transform', `translate(${screenX}, ${screenY}) rotate(${rotation})`);
        ...
        if (xEl)
            xEl.textContent = Math.round(this.x);
        if (yEl)
            yEl.textContent = Math.round(this.y);
        if (headingEl)
            headingEl.textContent = Math.round(this.heading);
        if (penEl)
            penEl.textContent = this.penDown ? 'Down' : 'Up';
        ...
        if (this.is3DMode) {
            ...
            if (zEl)
                zEl.textContent = Math.round(this.z);
            if (pitchEl)
                pitchEl.textContent = Math.round(this.pitch);
            if (rollEl)
                rollEl.textContent = Math.round(this.roll);
            this.updateTurtle3D();
        }
    }
    forward(distance) {
        ...
        if (this.is3DMode) {
            ...
            if (this.penDown) {
                this.drawLine3D(startX, startY, startZ, this.x, this.y, this.z);
            }
        }
        else {
            // 2D movement - only use heading
    ...
    collectExpressionTokens(tokens, startIndex) {
        ...
        const keywords = ['FORWARD', 'FD', 'BACKWARD', 'BK', 'BACK', 'LEFT', 'LT', 'RIGHT', 'RT',
            ...
            'IF', 'IFELSE', 'TO', 'END', 'PRINT', 'PR', 'WRITE', 'WR', 'APPLY', 'MAP', 'FILTER', 'REDUCE', 'INCLUDE'];
    ...
    parsePrimary(tokens, index) {
        ...
        if (['SQRT', 'SIN', 'COS', 'TAN', 'ABS', 'ROUND', 'FLOOR', 'CEILING', 'RANDOM', 'SPHERE', 'CUBE'].includes(func)) {
            ...
            switch (func) {
                ...
                case 'RANDOM':
                    ...
                    break;
                case 'SPHERE':
                    result = this.sphere(arg);
                    break;
                case 'CUBE':
                    result = this.cube(arg);
                    break;
            }
        ...
        // Two-argument 3D functions
        if (['CYLINDER', 'UNION', 'SUBTRACT', 'INTERSECT'].includes(func)) {
            const { value: arg1, nextIndex: afterArg1 } = this.parsePrimary(tokens, index + 1);
            const { value: arg2, nextIndex } = this.parsePrimary(tokens, afterArg1);
            let result;
            switch (func) {
                case 'CYLINDER':
                    result = this.cylinder(arg1, arg2);
                    break;
                case 'UNION':
                    result = this.csgUnion(arg1, arg2);
                    break;
                case 'SUBTRACT':
                    result = this.csgSubtract(arg1, arg2);
                    break;
                case 'INTERSECT':
                    result = this.csgIntersect(arg1, arg2);
                    break;
            }
            return { value: result, nextIndex };
        }
        // Query functions (no arguments)
        if (['XCOR', 'YCOR', 'ZCOR', 'HEADING', 'PENDOWN?', 'PENDOWNP', 'PENSIZE', 'PENCOLOR', 'REPCOUNT', 'LASTMESH'].includes(func)) {
            let result;
            switch (func) {
                case 'XCOR':
                    result = this.x;
        ...
        if (['FIRST', 'LAST', 'BUTFIRST', 'BF', 'BUTLAST', 'BL', 'COUNT', 'EMPTY?', 'EMPTYP'].includes(func)) {
            ...
            switch (func) {
                ...
                case 'LAST':
                    if (list.length === 0)
                        throw new Error('LAST: list is empty');
        ...
        if (func === 'LIST') {
            ...
            while (i < tokens.length) {
                ...
                const keywords = ['FORWARD', 'FD', 'BACKWARD', 'BK', 'LEFT', 'LT', 'RIGHT', 'RT',
                    ...
                    'APPLY', 'MAP', 'FILTER', 'REDUCE', 'INCLUDE'];
    ...
    drawLine(x1, y1, x2, y2) {
        if (this.is3DMode) {
            // Draw 3D line
            this.drawLine3D(x1, y1, this.z, x2, y2, this.z);
        }
        else {
            // Draw 2D SVG line
    ...
    init3D() {
        ...
        if (!container)
            return; // Skip if DOM element not available
        // Create scene
        this.scene3D = new THREE.Scene();
        this.scene3D.background = new THREE.Color(0x2a2a2a);
        // Create camera
        this.camera3D = new THREE.PerspectiveCamera(75, 1, 0.1, 10000);
        this.camera3D.position.set(400, 400, 400);
    ...
    toggle3DMode() {
        ...
        const canvas2D = document.getElementById('canvas');
        const canvas3D = document.getElementById('canvas3D');
        const toggleButton = document.getElementById('toggle3DButton');
        const exportSTLButton = document.getElementById('exportSTLButton');
        const zContainer = document.getElementById('turtleZContainer');
        const pitchContainer = document.getElementById('turtlePitchContainer');
        const rollContainer = document.getElementById('turtleRollContainer');
        if (this.is3DMode) {
            ...
            canvas2D.style.display = 'none';
            canvas3D.style.display = 'block';
            toggleButton.textContent = '2D Mode';
            exportSTLButton.style.display = 'inline-block';
            // Show Z, pitch, roll in turtle info
            zContainer.style.display = 'inline';
            pitchContainer.style.display = 'inline';
            rollContainer.style.display = 'inline';
        }
        else {
            // Show 2D canvas, hide 3D
            canvas2D.style.display = 'block';
            canvas3D.style.display = 'none';
            toggleButton.textContent = '3D Mode';
            exportSTLButton.style.display = 'none';
            // Hide Z, pitch, roll
    ...
    drawLine3D(x1, y1, z1, x2, y2, z2) {
        const material = new THREE.LineBasicMaterial({
            color: new THREE.Color(this.penColor),
            linewidth: this.penSize // Note: linewidth > 1 doesn't work in most browsers with WebGL
        });
        const points = [];
    ...
    beginFace() {
        // Start recording vertices for a face - add current position as first vertex
        this.faceVertices = [[this.x, this.y, this.z]];
    }
    endFace() {
        // Create a face from recorded vertices
        if (this.faceVertices.length < 3) {
            throw new Error('ENDFACE: Need at least 3 vertices to create a face');
        }
        if (!this.is3DMode || !this.scene3D) {
            this.log('Warning: ENDFACE only works in 3D mode');
            return;
        }
        // Create geometry from vertices
        const geometry = new THREE.BufferGeometry();
        const vertices = new Float32Array(this.faceVertices.flat());
        geometry.setAttribute('position', new THREE.BufferAttribute(vertices, 3));
        // Triangulate if more than 3 vertices (simple fan triangulation)
        if (this.faceVertices.length > 3) {
            const indices = [];
            for (let i = 1; i < this.faceVertices.length - 1; i++) {
        ...
        const material = new THREE.MeshBasicMaterial({
            color: new THREE.Color(this.penColor),
            side: THREE.DoubleSide // Show both sides of face
        });
        const mesh = new THREE.Mesh(geometry, material);
        this.scene3D.add(mesh);
        this.meshHistory.push(mesh); // Track for CSG operations
        this.lastMesh = mesh; // Store reference to last created mesh
        // Clear vertices for next face
        this.faceVertices = [];
    }
    sphere(radius) {
        if (!this.is3DMode || !this.scene3D) {
            this.log('Warning: SPHERE only works in 3D mode');
            return;
        }
        ...
        this.meshHistory.push(mesh); // Track for CSG operations
        this.lastMesh = mesh; // Store reference to last created mesh
        return mesh; // Return mesh reference for use in expressions
    }
    cube(size) {
        if (!this.is3DMode || !this.scene3D) {
            this.log('Warning: CUBE only works in 3D mode');
            return;
        }
    ...
    cylinder(radius, height) {
        if (!this.is3DMode || !this.scene3D) {
            this.log('Warning: CYLINDER only works in 3D mode');
            return;
        }
    ...
    exportSTL() {
        if (!this.is3DMode || !this.scene3D) {
            this.log('Error: Must be in 3D mode to export STL');
            return;
        }
        // Collect all meshes from the scene (excluding turtle, lights, helpers)
        const meshes = [];
        this.scene3D.children.forEach(child => {
            if (child.type === 'Mesh' && child !== this.turtle3D) {
                meshes.push(child);
            }
        });
        if (meshes.length === 0) {
            this.log('Warning: No meshes to export. Use BEGINFACE/ENDFACE to create faces.');
            return;
        }
        // Create a group containing all meshes
        const group = new THREE.Group();
        meshes.forEach(mesh => group.add(mesh.clone()));
        // Export to STL using Three.js STLExporter
        const exporter = new THREE.STLExporter();
        const stlString = exporter.parse(group, { binary: false }); // ASCII format is more compatible
        // Create download link
        const blob = new Blob([stlString], { type: 'text/plain' });
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = 'logo-model.stl';
        link.click();
        this.log(`Exported ${meshes.length} mesh(es) to logo-model.stl`);
    }
    csgUnion(mesh1, mesh2) {
        if (!this.is3DMode || !this.scene3D) {
            this.log('Warning: CSG operations only work in 3D mode');
            return null;
        }
        if (!mesh1 || !mesh2) {
            throw new Error('UNION requires two mesh arguments');
        }
        // Perform union using three-bvh-csg
        const evaluator = new window.ThreeBVH.Evaluator();
        const result = evaluator.evaluate(mesh1, mesh2, window.ThreeBVH.ADDITION);
        // Add to scene
        this.scene3D.add(result);
        this.meshHistory.push(result);
        this.lastMesh = result;
        // Remove original meshes from scene
        this.scene3D.remove(mesh1);
        this.scene3D.remove(mesh2);
        return result;
    }
    csgSubtract(mesh1, mesh2) {
        if (!this.is3DMode || !this.scene3D) {
            this.log('Warning: CSG operations only work in 3D mode');
            return null;
        }
        if (!mesh1 || !mesh2) {
            throw new Error('SUBTRACT requires two mesh arguments');
        }
        // Perform subtraction using three-bvh-csg
        const evaluator = new window.ThreeBVH.Evaluator();
        const result = evaluator.evaluate(mesh1, mesh2, window.ThreeBVH.SUBTRACTION);
        // Add to scene
        this.scene3D.add(result);
        this.meshHistory.push(result);
        this.lastMesh = result;
    ...
    csgIntersect(mesh1, mesh2) {
        if (!this.is3DMode || !this.scene3D) {
            this.log('Warning: CSG operations only work in 3D mode');
            return null;
        }
        if (!mesh1 || !mesh2) {
            throw new Error('INTERSECT requires two mesh arguments');
        }
        // Perform intersection using three-bvh-csg
        const evaluator = new window.ThreeBVH.Evaluator();
        const result = evaluator.evaluate(mesh1, mesh2, window.ThreeBVH.INTERSECTION);
        // Add to scene
        this.scene3D.add(result);
        this.meshHistory.push(result);
        this.lastMesh = result;
        ...
        while (i < end) {
            ...
            try {
                switch (token) {
                    ...
                    case 'SETXYZ':
                        {
                            const x = this.getNextValue(tokens, i + 1);
                            const y = this.getNextValue(tokens, x.nextIndex);
                            const z = this.getNextValue(tokens, y.nextIndex);
                            this.x = x.value;
                            this.y = y.value;
                            this.z = z.value;
                            this.updateTurtleDisplay();
                    ...
                    case 'SPHERE':
                        {
                            const { value: radius, nextIndex } = this.getNextValue(tokens, i + 1);
                            this.sphere(radius);
                            i = nextIndex - 1;
                        }
                        break;
                    case 'CUBE':
                        {
                            const { value: size, nextIndex } = this.getNextValue(tokens, i + 1);
                            this.cube(size);
                            i = nextIndex - 1;
                        }
                        break;
                    case 'CYLINDER':
                        {
                            const { value: radius, nextIndex: afterRadius } = this.getNextValue(tokens, i + 1);
                            const { value: height, nextIndex } = this.getNextValue(tokens, afterRadius);
                    ...
                    case 'INCLUDE':
                        {
                            // Get URL from next token (strip quotes if present)
                            let url = tokens[++i];
                            if (url.startsWith('"')) {
                                url = url.substring(1);
                            }
                            // Check for circular dependency
                            if (this.includedURLs.has(url)) {
                                const chain = [...this.includeStack, url].join(' -> ');
                                throw new Error(`Circular INCLUDE detected: ${chain}`);
                            }
                            try {
                                // Add to tracking before fetching
                                this.includedURLs.add(url);
                                this.includeStack.push(url);
                                ...
                                // Tokenize and execute the included code
                                const includedTokens = this.tokenize(code);
                                await this.execute(includedTokens);
                                this.log(`Included: ${url}`);
                            }
                            catch (error) {
                                throw new Error(`INCLUDE failed for ${url}: ${error.message}`);
                            }
                            finally {
                                // Pop from stack after execution (successful or not)
    ...
    async run(code) {
        ...
        this.includedURLs.clear(); // Reset include tracking for new run
        this.includeStack = []; // Reset include stack
        try {
...
`;
    ...
    // Color palette click handler (optional - only if palette exists)
    ...
    exportSTLButton.addEventListener('click', () => {
        interpreter.exportSTL();
    });
});
//# sourceMappingURL=logo-interpreter.js.map